...
export default function LabUpload(props) {
    ...
    const handleComponents = (e) => {
        if (e.target.value === '') {
            ...
            axios.get(apiPath + `/findlabcomponents/${e.target.value}`)
                .then(response => {
                    let Type
                    let SubType
    ...
    function uploadFile() {
        ...
        axios.get(apiPath + `/checklabfiles/${username}/${subject}/${Type}/${SubType}/${filename}`)
            .then(response => {
                let allcourses = []
                ...
                if (allcourses != '') {
                    ...
                    axios.post(apiPath + "/uploadlabfile", data, {
                        onUploadProgress: ProgressEvent => {
    ...
    if (loadingUser === false) {
        return (<Container component="main" maxWidth="s" className="card">
            ...
            <p style={{ marginTop: 20, marginBottom: 20 }}>Upload Lab Components Here</p>
